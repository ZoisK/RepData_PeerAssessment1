...
`r dim(data)[1]` observations of `r dim(data)[2]` variables
The variables are: `r names(data)`
...
`r format(dailysteps.mean, scientific=FALSE)` 

2. The median number of steps taken per day is
`r format(dailysteps.median, scientific=FALSE)`
...
contained in the 5-minute interval at `r maxsteps` 
...
1. There are in total `r missingvals` missing values in the dataset*
...
`r new.missingvals` missing values in the dataset

4. The calculated values for the data after imputing missing values are
* The mean number of steps taken per day is
`r format(new.dailysteps.mean, scientific=FALSE)` 
...
`r format(new.dailysteps.median, scientific=FALSE)`
...
`r sum( newdata$day == "weekday")` measurements on a weekday and 

`r sum( newdata$day == "weekend")` measurements on a weekend
